...
/** The agave rosette avatar — mood drives the CSS animation state. */
function AgaveAvatar({ mood }: { mood: Mood }) {
...
/**
 * Nada — the talking agave. Conversational (remembers the thread), listens
 * via the browser's speech recognition, and speaks answers back with the
 * built-in synthesizer. Degrades gracefully where voice isn't supported.
 */
export function NadaAssistant() {
  ...
  async function send(text: string) {
    ...
    setMood("thinking");
    const r = (await postJson("/api/ask", { question, history })) as { ok: boolean; answer?: string; error?: string };
    if (r.ok && r.answer) {
  ...
  return (
    <div className="mb-5 overflow-hidden rounded-lg border border-agave/25 bg-ink shadow-md">
      <div className="flex flex-col gap-4 p-4 sm:flex-row sm:items-stretch">
        <div className="flex flex-col items-center justify-center gap-2 sm:w-44">
          <AgaveAvatar mood={mood} />
          ...
          <button
            ...
            className={`rounded-full border px-2.5 py-0.5 text-[10px] transition-colors ${
              voiceOn ? "border-agave bg-agave/20 text-cream" : "border-cream/20 text-cream/50 hover:text-cream"
            }`}
          >
        ...
        <div className="flex min-h-56 flex-1 flex-col">
          <div ref={scroller} className="flex-1 space-y-2 overflow-y-auto pr-1" style={{ maxHeight: "280px" }}>
            {turns.length === 0 && (
              <div className="rounded-md bg-white/5 px-3 py-2 text-sm leading-relaxed text-cream/80">
                ...
                Follow-ups welcome; I remember the thread.
              </div>
            )}
            {turns.map((t, i) => (
              <div key={i} className={`flex ${t.role === "user" ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[85%] whitespace-pre-wrap rounded-lg px-3 py-2 text-sm leading-relaxed ${
                    t.role === "user" ? "bg-agave text-cream" : "bg-cream text-ink"
                  }`}
                >
                  {t.content}
                </div>
            ...
            {micSupported && (
              <button
                ...
                className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full transition-colors ${
                  mood === "listening" ? "bg-burnt text-cream" : "bg-agave text-cream hover:bg-agave-deep"
                }`}
              >
